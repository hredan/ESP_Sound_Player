...
void HandleWebpage::setCallBackSetMaxGain(
    CallBackSetMaxGain callBackSetMaxGain) {
...
bool HandleWebpage::loadFromLittleFS(String path) {
  ...
  if (LittleFS.exists(path)) {
    ...
    // Serial.println("Error: Path does not exist and will be redirect to
    // root:");
...
void HandleWebpage::setupHandleWebpage() {
  ...
  _webServer->on("/getData", HTTP_GET,
                 std::bind(&HandleWebpage::handleGetData, this));
  _webServer->on("/playSound", HTTP_POST,
                 std::bind(&HandleWebpage::handlePlaySound, this));
  _webServer->on("/stopSound", HTTP_POST,
                 std::bind(&HandleWebpage::handleStopSound, this));
  _webServer->on("/setMaxGain", HTTP_POST,
                 std::bind(&HandleWebpage::handleSetMaxGain, this));
  _webServer->on("/saveData", HTTP_POST,
                 std::bind(&HandleWebpage::handleSaveData, this));
  _webServer->begin();
}
