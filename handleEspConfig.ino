...
EspLedRingConfig getLedRingConfig()
{
    ...
    if (tryReadIntFromJson(configJson, "ledRingLedCount", ledCount) && ledCount > 0)
    {
        config.ledCount = static_cast<uint16_t>(ledCount);
    }

    int brightness = config.brightness;
    if (tryReadIntFromJson(configJson, "ledRingBrightness", brightness) && brightness >= 0 && brightness <= 255)
    {
