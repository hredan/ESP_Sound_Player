...
float ReactiveAudioOutputI2S::getLevel() const { return _level; }
...
void applyConfiguredPinout(AudioOutputI2S* out) {
  ...
  if (!readEspConfigJson(configJson)) {
    Serial.println(
        "No config file found or readable: skip AudioOutputI2S.SetPinout");
    return;
  }
  ...
  bool hasPinout = tryReadIntFromJson(configJson, "i2sBclk", bclk) &&
  ...
  Serial.printf("AudioOutputI2S.SetPinout %d (bclk=%d, wclk=%d, dout=%d)\n",
                out->SetPinout(bclk, wclk, dout), bclk, wclk, dout);
}
}  // namespace
...
void HandleAudio::setMaxGain(float maxGain) { _maxGain = maxGain; }
