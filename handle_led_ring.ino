...
HandleLedRing::HandleLedRing(uint16_t ledCount, uint8_t dataPin,
                             uint8_t brightness)
...
uint32_t HandleLedRing::colorFromAudioLevel(float level) const {
  ...
  uint8_t red =
      static_cast<uint8_t>(constrain((level - 0.25f) * 340.0f, 0.0f, 255.0f));
  uint8_t green = static_cast<uint8_t>(
      constrain((1.0f - fabsf(level - 0.5f) * 2.0f) * 255.0f, 0.0f, 255.0f));
  uint8_t blue =
      static_cast<uint8_t>(constrain((1.0f - level) * 255.0f, 0.0f, 255.0f));
...
uint8_t HandleLedRing::brightnessFromAudioLevel(float level) const {
  ...
  return static_cast<uint8_t>(minBrightness +
                              (maxBrightness - minBrightness) * level);
}
